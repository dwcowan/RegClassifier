...
if ~isempty(external_rules) && length(external_rules) > 0 %#ok<ISMT>
    ...
    % Default keyword rules (expanded; matches weak_rules.m)
...
% same chunk to avoid false positives.
...
for j = 1:K
    ...
    for i = 1:N
        ...
        for p = 1:numel(patterns)
            ...
            % Co-occurrence gating: ambiguous short keywords require a
            % context word in the same chunk to confirm regulatory meaning.
            if isKey(cooccur, pattern_char)
                ctxWords = cooccur(pattern_char);
                hasCtx = false;
                for ci = 1:numel(ctxWords)
                    ctxPat = ['\b', regexptranslate('escape', char(ctxWords(ci))), '\b'];
                    if ~isempty(regexp(text, ctxPat, 'once'))
                        hasCtx = true;
                        break;
                    end
                end
                if ~hasCtx
                    continue;  % Skip: ambiguous keyword without context
                end
            end
