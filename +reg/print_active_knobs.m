...
if isfield(C,'knobs') && ~isempty(C.knobs)
    ...
    if isfield(K,'Projection')
        fprintf("Projection: ProjDim=%s, BatchSize=%s, Epochs=%s, LR=%s, Margin=%s, UseGPU=%s\n", ...
            getf(K.Projection,'ProjDim'), getf(K.Projection,'BatchSize'), ...
            getf(K.Projection,'Epochs'), getf(K.Projection,'LR'), getf(K.Projection,'Margin'), getf(K.Projection,'UseGPU'));
    end
