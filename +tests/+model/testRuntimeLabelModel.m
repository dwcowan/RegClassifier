classdef testRuntimeLabelModel < matlab.unittest.TestCase
    %% Test suite for reg.model.RuntimeLabelModel
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.model.RuntimeLabelModel(), 'reg:model:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.model.RuntimeLabelModel','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.model.RuntimeLabelModel');
            if ~isempty(missing)
