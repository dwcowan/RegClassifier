classdef testDatabaseModel < matlab.unittest.TestCase
    %% Test suite for reg.model.DatabaseModel
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.model.DatabaseModel(), 'reg:model:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.model.DatabaseModel','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.model.DatabaseModel');
            if ~isempty(missing)
