classdef testCorpusModel < matlab.unittest.TestCase
    %% Test suite for reg.model.CorpusModel
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.model.CorpusModel(), 'reg:model:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.model.CorpusModel','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.model.CorpusModel');
            if ~isempty(missing)
