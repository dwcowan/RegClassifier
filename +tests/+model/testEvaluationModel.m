classdef testEvaluationModel < matlab.unittest.TestCase
    %% Test suite for reg.model.EvaluationModel
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.model.EvaluationModel(), 'reg:model:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.model.EvaluationModel','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.model.EvaluationModel');
            if ~isempty(missing)
