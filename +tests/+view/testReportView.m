classdef testReportView < matlab.unittest.TestCase
    %% Test suite for reg.view.ReportView
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.view.ReportView(), 'reg:view:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.view.ReportView','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.view.ReportView');
            if ~isempty(missing)
