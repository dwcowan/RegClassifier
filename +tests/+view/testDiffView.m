classdef testDiffView < matlab.unittest.TestCase
    %% Test suite for reg.view.DiffView
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.view.DiffView(), 'reg:view:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.view.DiffView','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.view.DiffView');
            if ~isempty(missing)
