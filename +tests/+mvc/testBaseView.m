classdef testBaseView < matlab.unittest.TestCase
    %% Test suite for reg.mvc.BaseView
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.mvc.BaseView(), 'reg:mvc:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.mvc.BaseView','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.mvc.BaseView');
            if ~isempty(missing)
