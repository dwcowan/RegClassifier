classdef testBaseModel < matlab.unittest.TestCase
    %% Test suite for reg.mvc.BaseModel
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.mvc.BaseModel(), 'reg:mvc:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.mvc.BaseModel','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.mvc.BaseModel');
            if ~isempty(missing)
