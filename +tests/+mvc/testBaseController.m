classdef testBaseController < matlab.unittest.TestCase
    %% Test suite for reg.mvc.BaseController
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.mvc.BaseController(), 'reg:mvc:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.mvc.BaseController','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.mvc.BaseController');
            if ~isempty(missing)
