classdef testCorpusController < matlab.unittest.TestCase
    %% Test suite for reg.controller.CorpusController
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.controller.CorpusController(), 'reg:controller:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.controller.CorpusController','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.controller.CorpusController');
            if ~isempty(missing)
