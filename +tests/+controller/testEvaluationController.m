classdef testEvaluationController < matlab.unittest.TestCase
    %% Test suite for reg.controller.EvaluationController
    ...
    methods (Test, TestTags={'Unit'})
        function constructorStub(testCase)
            testCase.verifyError(@() reg.controller.EvaluationController(), 'reg:controller:NotImplemented');
        end
    end
    ...
    methods (Test, TestTags={'Integration'})
        function methodPresence(testCase)
            ...
            if ~utils.methodExists('reg.controller.EvaluationController','run')
    ...
    methods (Test, TestTags={'Smoke'})
        function contractsAvailable(testCase)
            utils = tests.fixtures.reflect_utils;
            missing = utils.missingArgumentsBlocks('reg.controller.EvaluationController');
            if ~isempty(missing)
